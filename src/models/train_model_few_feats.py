...
keep_only_cols = ['_Max`Prod`(BOE)', 'Field', 'Surf_Longitude', 'Surf_Latitude', 'HZLength', 'TotalDepth',
                  'TVD', '_Fracture`Stages',
...
def main(input_file_path, output_file_path, tgt='Oil_norm', n_splits=5):
    ...
    for train_index, test_index in cv.split(X):
        ...
        model.fit(X_train, y_train, categorical_feature=['Field','WellTypeStandardised'])
