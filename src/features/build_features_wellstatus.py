...
for s in sets:
    if s == 'Test':
        ...
        ws_data = pd.read_csv(well_status_file_name).loc[:, ['EPAssetsId', 'well_status_code']]
    else:
        well_status_file_name = f'/home/anton/Repos/untapped-nrj-classification/data/final/{s}_final.pck'
        ws_data = pd.read_pickle(well_status_file_name).loc[:, ['EPAssetsId', 'well_status_code']]
    dfs.append(ws_data)
